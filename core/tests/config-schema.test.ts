...
export function createValidConfig(overrides: Partial<CoreConfig> = {}): CoreConfig {
  const base = {
    ...
    imageDescription: {
      ...
      detail: "low",
      prompt:
        "请为另一个只能读取文本的聊天模型描述这张图片。请客观描述可见内容、人物或物体、动作、场景、表情、画面中的文字，如果这是表情包、梗图或截图，也请说明它传达的语气和关键上下文。输出简洁但信息充分，不要臆测看不见的细节。",
      maxOutputTokens: 256,
    },
    hookTimeouts: {
      tool: 3000,
...
describe("Config schema validation", () => {
  it("accepts the full default-like core config", () => {
    ...
    expect(validated.imageDescription.detail).toBe("low");
  ...
  it("accepts image description mode with dedicated multimodal settings", () => {
    const validated = Config(
      createValidConfig({
    ...
    expect(validated.imageMode).toBe("description");
    expect(validated.imageDescription.model).toBe("openai:gpt-4o-mini");
    expect(validated.imageDescription.fallbackChain).toEqual(["google:gemini-2.5-flash"]);
    expect(validated.imageDescription.detail).toBe("high");
    expect(validated.imageDescription.prompt).toBe("Describe the image for a text-only model.");
    expect(validated.imageDescription.maxOutputTokens).toBe(384);
  });
