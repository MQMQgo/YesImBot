...
export class ThinkActLoop {
  ...
  async run(
    ...
    try {
      ...
      const imageConfig = {
        imageMode: this.config.imageMode ?? "native",
        maxImagesInContext: this.config.maxImagesInContext ?? 3,
        imageLifecycleCount: this.config.imageLifecycleCount ?? 3,
        description: {
          model: this.config.imageDescription?.model ?? "",
          fallbackChain: this.config.imageDescription?.fallbackChain ?? [],
          detail:
            this.config.imageDescription?.detail ?? DEFAULT_IMAGE_DESCRIPTION_DETAIL,
          prompt:
            this.config.imageDescription?.prompt ?? DEFAULT_IMAGE_DESCRIPTION_PROMPT,
          maxOutputTokens:
            this.config.imageDescription?.maxOutputTokens ??
            DEFAULT_IMAGE_DESCRIPTION_MAX_OUTPUT_TOKENS,
        },
      };
